...
class VariationalAutoencoder(object):
    ...
    def _initialize_weights(self, encoder_1, encoder_2, encoder_3, encoder_4,
                            decoder_1, decoder_2, decoder_3, decoder_4,
                            n_input, latent_z):
        all_weights = dict()
        with tf.variable_scope("weights_encoder"):
            all_weights['weights_encoder'] = {
                ...
                'h2': tf.get_variable('h2', shape=[encoder_1, encoder_2],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'h3': tf.get_variable('h3', shape=[encoder_2, encoder_3],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'h4': tf.get_variable('h4', shape=[encoder_3, encoder_4],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'out_mean': tf.get_variable('out_mean', shape=[encoder_4, latent_z],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'out_log_sigma': tf.get_variable('out_log_sigma', shape=[encoder_4, latent_z],
                                            initializer=tf.contrib.layers.xavier_initializer())}
        all_weights['biases_encoder'] = {
            ...
            'b3': tf.Variable(tf.zeros([encoder_3], dtype=tf.float32)),
            'b4': tf.Variable(tf.zeros([encoder_4], dtype=tf.float32)),
        ...
        with tf.variable_scope("weights_decoder"):
            all_weights['weights_decoder'] = {
                ...
                'h2': tf.get_variable('h2', shape=[decoder_1, decoder_2],
                                            initializer=tf.contrib.layers.xavier_initializer()),
                'h3': tf.get_variable('h3', shape=[decoder_2, decoder_3],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'h4': tf.get_variable('h4', shape=[decoder_3, decoder_4],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'out_mean': tf.get_variable('out_mean', shape=[decoder_4, n_input],
                                            initializer=tf.contrib.layers.xavier_initializer()),
                'out_log_sigma': tf.get_variable('out_log_sigma', shape=[decoder_4, n_input],
                                            initializer=tf.contrib.layers.xavier_initializer())}
        all_weights['biases_decoder'] = {
            ...
            'b3': tf.Variable(tf.zeros([decoder_3], dtype=tf.float32)),
            'b4': tf.Variable(tf.zeros([decoder_3], dtype=tf.float32)),
    ...
    def _encoder_network(self, weights, biases):
        ...
        layer_2 = self.activation_fct(tf.add(tf.matmul(layer_1, weights['h2']),
                                           biases['b2']))
        layer_3 = self.activation_fct(tf.add(tf.matmul(layer_2, weights['h3']),
                                             biases['b3']))
        layer_4 = self.activation_fct(tf.add(tf.matmul(layer_3, weights['h4']),
                                             biases['b4']))
...
network_architecture = \
    dict(encoder_1=500,  # 1st layer encoder neurons
         ...
         encoder_3=500,  # 3rd layer encoder neurons
         encoder_4=500,  # 3rd layer encoder neurons
