...
class VariationalAutoencoder(object):
    ...
    def _initialize_weights(self, encoder_1, encoder_2, encoder_3,
        ...
        with tf.variable_scope("weights_encoder"):
            all_weights['weights_encoder'] = {
                ...
                'h3': tf.get_variable('h3', shape=[encoder_2, encoder_3],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'out_mean': tf.get_variable('out_mean', shape=[encoder_3, latent_z],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'out_log_sigma': tf.get_variable('out_log_sigma', shape=[encoder_3, latent_z],
                                            initializer=tf.contrib.layers.xavier_initializer())}
        ...
        with tf.variable_scope("weights_decoder"):
            all_weights['weights_decoder'] = {
                ...
                'h3': tf.get_variable('h3', shape=[decoder_2, decoder_3],
                                      initializer=tf.contrib.layers.xavier_initializer()),
                'out_mean': tf.get_variable('out_mean', shape=[decoder_3, n_input],
                                            initializer=tf.contrib.layers.xavier_initializer()),
                'out_log_sigma': tf.get_variable('out_log_sigma', shape=[decoder_3, n_input],
                                            initializer=tf.contrib.layers.xavier_initializer())}
...
network_architecture = \
    dict(encoder_1=500,  # 1st layer encoder neurons
         encoder_2=400,  # 2nd layer encoder neurons
         encoder_3=300,  # 3rd layer encoder neurons
         decoder_1=500,  # 1st layer decoder neurons
         decoder_2=400,  # 2nd layer decoder neurons
         decoder_3=300,  # 3nd layer decoder neurons
